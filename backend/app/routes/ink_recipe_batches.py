from decimal import Decimal, InvalidOperation

from flask import Blueprint, jsonify, request
from flask_jwt_extended import jwt_required
from sqlalchemy.exc import IntegrityError

from app.database import SessionLocal
from app.models.ink_recipe_batch import BATCH_STATUSES, BATCH_TRANSITIONS, InkRecipeBatch
from app.models.workshop import Workshop
from app.serializers import ink_recipe_batch_json
from app.utils import error

bp = Blueprint("ink_recipe_batches", __name__, url_prefix="/api/ink-recipe-batches")

_STATUS_LABELS = {
    "draft": "草稿",
    "mixing": "调墨中",
    "qc_pass": "质检合格",
    "scrap": "报废",
}


def _validate_fields(body: dict) -> tuple[dict | None, str | None]:
    workshop_id = int(body.get("workshopId") or 0)
    if workshop_id <= 0:
        return None, "请选择所属车间"

    batch_code = str(body.get("batchCode", "")).strip()
    if not batch_code:
        return None, "批次编号不能为空"

    pigment_base = str(body.get("pigmentBase", "")).strip()
    if not pigment_base:
        return None, "色浆基料不能为空"

    raw_viscosity = str(body.get("targetViscosityPaS", "")).strip()
    if not raw_viscosity:
        return None, "目标粘度(Pa·s)不能为空"
    try:
        target_viscosity = Decimal(raw_viscosity)
    except (InvalidOperation, ValueError):
        return None, "目标粘度(Pa·s)必须为正数"
    if target_viscosity <= 0:
        return None, "目标粘度(Pa·s)必须为正数"

    note_raw = body.get("note")
    note = str(note_raw).strip() if note_raw is not None else None
    if note == "":
        note = None

    db = SessionLocal()
    try:
        if not db.get(Workshop, workshop_id):
            return None, "所属车间不存在"
    finally:
        db.close()

    return (
        {
            "workshop_id": workshop_id,
            "batch_code": batch_code,
            "pigment_base": pigment_base,
            "target_viscosity_pa_s": target_viscosity,
            "note": note,
        },
        None,
    )


@bp.get("")
@jwt_required()
def list_batches():
    workshop_id = request.args.get("workshopId", type=int)
    status = (request.args.get("status") or "").strip()

    if status and status not in BATCH_STATUSES:
        return error("状态无效，应为 draft / mixing / qc_pass / scrap", 400)

    db = SessionLocal()
    try:
        query = db.query(InkRecipeBatch)
        if workshop_id:
            query = query.filter(InkRecipeBatch.workshop_id == workshop_id)
        if status:
            query = query.filter(InkRecipeBatch.status == status)
        rows = query.order_by(InkRecipeBatch.id.desc()).all()
        return jsonify([ink_recipe_batch_json(r) for r in rows])
    finally:
        db.close()


@bp.post("")
@jwt_required()
def create_batch():
    body = request.get_json(silent=True) or {}
    data, err = _validate_fields(body)
    if err:
        return error(err, 400)

    db = SessionLocal()
    try:
        row = InkRecipeBatch(status="draft", **data)
        db.add(row)
        try:
            db.commit()
        except IntegrityError:
            db.rollback()
            return error("该车间下批次编号已存在", 400)
        db.refresh(row)
        return jsonify(ink_recipe_batch_json(row)), 201
    finally:
        db.close()


@bp.get("/<int:item_id>")
@jwt_required()
def get_batch(item_id: int):
    db = SessionLocal()
    try:
        row = db.get(InkRecipeBatch, item_id)
        if not row:
            return error("配方批次不存在", 404)
        return jsonify(ink_recipe_batch_json(row))
    finally:
        db.close()


@bp.put("/<int:item_id>")
@jwt_required()
def update_batch(item_id: int):
    body = request.get_json(silent=True) or {}
    data, err = _validate_fields(body)
    if err:
        return error(err, 400)

    db = SessionLocal()
    try:
        row = db.get(InkRecipeBatch, item_id)
        if not row:
            return error("配方批次不存在", 404)

        row.workshop_id = data["workshop_id"]
        row.batch_code = data["batch_code"]
        row.pigment_base = data["pigment_base"]
        row.target_viscosity_pa_s = data["target_viscosity_pa_s"]
        row.note = data["note"]
        try:
            db.commit()
        except IntegrityError:
            db.rollback()
            return error("该车间下批次编号已存在", 400)
        db.refresh(row)
        return jsonify(ink_recipe_batch_json(row))
    finally:
        db.close()


@bp.delete("/<int:item_id>")
@jwt_required()
def delete_batch(item_id: int):
    db = SessionLocal()
    try:
        row = db.get(InkRecipeBatch, item_id)
        if not row:
            return error("配方批次不存在", 404)
        db.delete(row)
        db.commit()
        return jsonify({"ok": True})
    finally:
        db.close()


@bp.post("/<int:item_id>/transition")
@jwt_required()
def transition_batch(item_id: int):
    body = request.get_json(silent=True) or {}
    target = str(body.get("status") or "").strip()
    if target not in BATCH_STATUSES:
        return error("目标状态无效，应为 draft / mixing / qc_pass / scrap", 400)

    db = SessionLocal()
    try:
        row = db.get(InkRecipeBatch, item_id)
        if not row:
            return error("配方批次不存在", 404)

        current = row.status
        if target == current:
            return error(f"批次已处于「{_STATUS_LABELS[current]}」状态，无需重复流转", 409)
        if target not in BATCH_TRANSITIONS[current]:
            return error(
                f"非法状态流转：{_STATUS_LABELS[current]} → {_STATUS_LABELS[target]}", 409
            )

        row.status = target
        note = body.get("note")
        if note is not None:
            text = str(note).strip()
            row.note = text or None
        db.commit()
        db.refresh(row)
        return jsonify(ink_recipe_batch_json(row))
    finally:
        db.close()
